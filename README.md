...
### 2. 获得自然图像的文本描述 / Obtain a Textual Description of the Real Image
```bash
python img_to_text.py -f Imgs/Real -o Prompt.txt -c ViT-H-14/laion2b_s32b_b79k -m classic
```

### 3. 利用上述提示词生成图像 / Generate Images based on the Prompt
```bash
...
### 4. 计算两次重建的损失比 / Calculate the Loss Ratio between the Two Reconstructions
```bash
...
### 5. 计算图像同质性指标 / Calculate Image Homogeneity
```bash
